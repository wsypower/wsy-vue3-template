  ...
  <el-popover v-model:visible="data.visible" placement="bottom" trigger="hover" :width="250">
    ...
    <el-tabs ref="tabs" v-model="data.activeTab">
      <el-tab-pane label="Element Icon" name="element">
        ...
        <div class="list-icon">
          <span
            ...
            @click="choose(`ElIcon${item}`)"
          >
            <svg-icon :name="`el-icon-${item}`" />
          </span>
        </div>
        <el-pagination
          v-model:current-page="data.element.currentPage"
          small
          layout="prev, pager, next"
          :page-size="data.element.pageSize"
      ...
      <el-tab-pane label="Remix Icon" name="remix">
        ...
        <div class="list-icon">
          <span
            v-for="(item, index) in remixIconCurrentList"
            :key="index"
            class="list-icon-item"
            @click="
              choose(
                item.type == 'Editor' ? `ri-${item.name}` : `ri-${item.name}-${data.remix.style}`
              )
            "
          >
            <svg-icon
        ...
        <div class="style-choose">
          <el-radio-group v-model="data.remix.style" size="mini">
            <el-radio-button label="line">线条</el-radio-button>
            <el-radio-button label="fill">填充</el-radio-button>
          </el-radio-group>
        </div>
        <el-pagination
          v-model:current-page="data.remix.currentPage"
          small
          layout="prev, pager, next"
          :page-size="data.remix.pageSize"
          :total="remixIconList.length"
          :pager-count="5"
        />
      </el-tab-pane>
    </el-tabs>
...
<script setup>
  ...
  const elementIconList = computed(() => {
    ...
    if (data.value.element.search != '') {
      list = list.filter((item) => {
        return (
          pascal(item).toLowerCase().indexOf(pascal(data.value.element.search).toLowerCase()) >= 0
        )
      })
    }
    return list
  ...
  const remixIconList = computed(() => {
    ...
    if (data.value.remix.search != '') {
      list = list.filter((item) => {
        return (
          item.keyword.includes(data.value.remix.search) ||
          item.name.indexOf(data.value.remix.search) >= 0
        )
      })
    }
    return list
