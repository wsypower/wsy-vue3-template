...
<script setup>
  ...
  function init() {
    ...
    dpr =
      typeof window !== 'undefined' && config.isDpr
  ...
  function setLineWidth() {
    ...
    let returnNum =
      config.minWriteWidth + ((config.maxWriteWidth - config.minWriteWidth) * diffTime) / 30
  ...
  function downloadSignImg(name) {
    ...
    saveFile(
      dataURL,
      name ? `${name}.${config.imgType}` : `${Date.parse(new Date())}.${config.imgType}`
    )
  }
  // 保存文件
