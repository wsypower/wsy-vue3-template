<template>
  <div class="layout">
    <div
      id="app-main"
      :class="{ 'main-page-maximize': $store.state.settings.mainPageMaximizeStatus }"
    >
      <Header />
      <div class="wrapper">
        <div
        ...
        <div class="main-container" :style="{ 'padding-bottom': $route.meta.paddingBottom }">
          <component
            :is="tabbarAndTopbarComponent(item)"
            v-for="(item, index) in tabbarAndTopbarOrder"
            :key="index"
          />
          <div class="main">
            <div
              v-show="$store.state.settings.mainPageMaximizeStatus"
              class="exit-main-page-maximize"
              @click="$store.commit('settings/setMainPageMaximize')"
            >
...
<script setup>
  ...
  const tabbarAndTopbarOrder = computed(() => {
    ...
    if (
      !(
        (store.state.settings.menuMode === 'head' &&
          !store.state.settings.enableSidebarCollapse &&
          !store.state.settings.enableBreadcrumb) ||
  ...
  const showCopyright = computed(() => {
    return typeof routeInfo.meta.copyright !== 'undefined'
      ? routeInfo.meta.copyright
      : store.state.settings.showCopyright
  })
...
<style lang="scss" scoped>
  ...
  [data-menu-mode='only-side'] {
    .sidebar-container {
      width: $g-main-sidebar-width;
    }

    .main-container {
      margin-left: $g-main-sidebar-width;
    }
  }

  [data-menu-mode='only-head'] {
    .sidebar-container {
      width: 0;
    }
