<template>
  ...
  >
    <Logo
      :show-logo="$store.state.settings.menuMode === 'single'"
      :class="{
        'sidebar-logo': true,
    ...
    <!-- 侧边栏模式（无主导航）或侧边栏精简模式 -->
    <el-menu
      :unique-opened="$store.state.settings.sidebarUniqueOpened"
      ...
      :class="{
        'is-collapse-without-logo':
          $store.state.settings.menuMode !== 'single' && $store.state.settings.sidebarCollapse,
