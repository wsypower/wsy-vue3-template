      ...
      <div ref="search" class="result" :class="{ mobile: $store.state.settings.mode === 'mobile' }">
        ...
        >
          <a
            ...
            :href="isExternal(item.path) ? item.path : href"
            class="item"
            :class="{ actived: index === actived }"
            :target="isExternal(item.path) ? '_blank' : '_self'"
            @click="navigate"
...
<script setup>
  ...
  const resultList = computed(() => {
    ...
    result = sourceList.value.filter((item) => {
      ...
      if (
        item.breadcrumb.some(
          (b) => generateI18nTitle(b.i18n, b.title).indexOf(searchInput.value) >= 0
        )
      ) {
        flag = true
      }
  ...
  function scrollTo(offsetTop) {
    if (actived.value !== -1) {
      if (
        offsetTop + proxy.$refs[`search-item-${actived.value}`].clientHeight >
